...
<!--
 ...
 -->
    ...
    <g:if test="${!readOnly}">
      <input type="file" name="${parameterName}" size="${size}" class="${styleClass}"/>
    </g:if>
    <g:if test="${variable.value}">
      <gf:renderDocument document="${variable.value}" label="${gf.translatedValue(translations: variable.description)}"/>
    </g:if>
